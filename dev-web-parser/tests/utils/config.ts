...
let passFailOverrideList: {
  ...
  "/paratextTests/WordlistMarkerMissingFromGlossaryCitationForms/origin.usfm":
    "pass",
...
function checkValidUsfm(inputUsfmPath: string) {
  ...
  if (inputUsfmPath.replace(TEST_DIR, "") in passFailOverrideList) {
    if (passFailOverrideList[inputUsfmPath.replace(TEST_DIR, "")] === "pass") {
      ...
      passFailOverrideList[inputUsfmPath.replace(TEST_DIR, "")] === "fail"
    ) {
      return false;
    }
  }
...
const findAllMarkers = function (
  ...
  let inputMarkers: string[] = [];
  if (keepNumber) {
    inputMarkers = allMarkersInInput.map((match) => match[1]);
  } else {
    inputMarkers = allMarkersInInput.map((match) => match[1] + match[3]);
  }

  // Remove duplicates
  inputMarkers = [...new Set(inputMarkers)];

  // Remove 'id' marker if `keepId` is false
  if (!keepId) {
    const idIndex = inputMarkers.indexOf("id");
    if (idIndex !== -1) inputMarkers.splice(idIndex, 1);
  }

  // Handle 'esbe' and 'usfm' markers
  const esbeIndex = inputMarkers.indexOf("esbe");
  if (esbeIndex !== -1) {
    const esbIndex = inputMarkers.indexOf("esb");
    if (esbIndex === -1)
      throw new Error("'esb' must be present if 'esbe' is found");
    inputMarkers.splice(esbeIndex, 1);
  }

  const usfmIndex = inputMarkers.indexOf("usfm");
  if (usfmIndex !== -1) {
    inputMarkers.splice(usfmIndex, 1);
  }
...
export {allUsfmFiles, isValidUsfm, excludeUSJs, excludeUSXs, findAllMarkers};
